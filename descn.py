...
class DESCN:
    ...
    def cal_loss(self):
        if self.use_sample_weight:
            w_outcome, w_treatment = self.y_w, self.t_w
        else:
            w_outcome, w_treatment = 1, 1

        y_outcome, y_treatment = self.y, self.t
        # loss for propensity
        self.prpsy_loss = w_treatment * self.prpsy_w * focal_loss(y_treatment, self.p_prpsy)
        # loss for ESTR ESCR
        self.estr_loss = (w_treatment + w_outcome) * self.escvr1_w * focal_loss(y_outcome * y_treatment, self.p_estr)
        self.escr_loss = w_outcome * self.escvr0_w * focal_loss(y_outcome * (1 - y_treatment), self.p_escr)

        # loss for TR, CR
        self.tr_loss = self.h1_w * focal_loss(y_outcome, self.p_h1) * w_outcome
        self.cr_loss = self.h0_w * focal_loss(y_outcome, self.p_h0) * w_outcome

        # loss for cross TR: mu1_prime, cross CR: mu0_prime
        self.cross_tr_loss = self.mu1hat_w * focal_loss(y_outcome,
                                                        tf.sigmoid(self.mu0_logit + self.tau_logit)) * w_outcome
        self.cross_cr_loss = self.mu0hat_w * focal_loss(y_outcome,
    ...
    def build_model(self, x=None, y=None, t=None, y_w=None, t_w=None, mode=None, hooks=None):
        ...
        with tf.variable_scope('train', reuse=tf.AUTO_REUSE):
            ...
            with tf.control_dependencies(update_ops):
                if mode == tf.estimator.ModeKeys.TRAIN:
                    ...
                    eval_metric_ops['pred_tr_auc'] = tf.metrics.auc(self.y, self.p_h1)
                    eval_metric_ops['pred_cr_auc'] = tf.metrics.auc(self.y, self.p_h0)
                    eval_metric_ops['pred_t_auc'] = tf.metrics.auc(self.t, self.p_prpsy)
...
if __name__ == '__main__':
    ...
    data_y_w = np.random.random_integers(1, 5, size=(batch_size, 1))
    data_t_w = np.random.random_integers(1, 5, size=(batch_size, 1))
    ...
    placeholder_t = tf.placeholder(dtype=tf.int64, shape=(None, 1))

    placeholder_y_w = tf.placeholder(dtype=tf.float32, shape=(None, 1))
    placeholder_t_w = tf.placeholder(dtype=tf.float32, shape=(None, 1))
    ...
    model.build_model(x=placeholder_x, y=placeholder_y, t=placeholder_t,
                      y_w=placeholder_y_w, t_w=placeholder_t_w,
